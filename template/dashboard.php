...
<div class="wrap">
    ...
    <h2 class="nav-tab-wrapper">
        <a href="?page=hmu_ajax_filter&tab=taxonomy_settings"
           class="nav-tab <?php echo ($_GET['tab'] == '' || $_GET['tab'] == 'taxonomy_settings')
               ? 'tab-active' : ''; ?>
        <?php echo ($_GET['tab'] == '') ? $_GET['tab'] : ''; ?>">Taxonomy settings</a>
        <a href="?page=hmu_ajax_filter&tab=global_settings"
           class="nav-tab <?php echo ($_GET['tab'] == 'global_settings')
               ? 'tab-active' : ''; ?>">Global settings</a>
    </h2>
            ...
            <div class="block--shop_filter_attributes" id="">
                <form action="<?php echo esc_url(admin_url('admin.php?page=hmu_ajax_filter')); ?>" method="post"
                    ...
                    foreach ($taxonomy_objects as $tax) {
                        ...
                        echo '<div class="tax-parent">
                              <label class="taxonomy-label parent-label"><strong>' . $tax . '</strong></label>';
                                        ...
                                        foreach ($chilterms as $childterm) {
                                            ...
                                            <div class="tax-children">
                                                ...
                                                <input
                                                        ...
                                                        name="<?php
                                                        echo $this->seoUrl($term->name)
                                                            . '[' . $this->seoUrl($childterm->name) . ']'; ?>"
                                                    <?php foreach ($option_terms as $key => $option_term) {
                                                        echo array_key_exists(
                                                            $this->seoUrl($childterm->name),
                                                            $option_term
                ...
                <div class="hmu_delete_wrapper">
                    <a class="hmu_delete"
                       href="<?php echo esc_url(admin_url('admin.php?page=hmu_ajax_filter&reset=true')); ?>">Reset to
                        default</a>
